...
char* make_path_to_parent(const char* path, char* component) {
    ...
    if (result == NULL) fatal("memory allocation failed");
...
bool is_a_prefix_of(const char *a, const char *b) {
    const char *a_ptr = a;
    const char *b_ptr = b;
...
char *last_common_ancestor(const char *path_a, const char *path_b) {
    char *result = malloc((max(strlen(path_a), strlen(path_b)) + 1) * sizeof(char));
    if (result == NULL) fatal("memory allocation failed");

    char component_a[MAX_FOLDER_NAME_LENGTH];
    char component_b[MAX_FOLDER_NAME_LENGTH];
    const char *subpath_a = path_a;
    const char *subpath_b = path_b;

    result[0] = '/';
    size_t result_index = 1;
    subpath_a = split_path(subpath_a, component_a);
    subpath_b = split_path(subpath_b, component_b);

    while (strcmp(component_a, component_b) == 0 && (subpath_a != NULL && subpath_b != NULL)) {
        strcpy(result + result_index, component_a);
        result_index += strlen(component_a);
        result[result_index++] = '/';
